...
* `transform`: Includes all the requirements for deploying the transform function to the Redpanda broker, including `rpk`, Go, and the redaction transform code. The source code for the redaction transform is available in the `redaction` directory.

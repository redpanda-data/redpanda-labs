...
// (test start {"id":"data-transform-iss-demo-go", "description": "Convert JSON Messages into Avro", "setup": "doc-tests/fetch-versions-and-rpk.json", "cleanup": "doc-tests/cleanup.json"})
...
./post-schema.sh
```
// (step {"action":"runShell", "command": "./post-schema.sh"})
...
```
// (step {"action":"runShell", "command": "rpk profile create iss_demo --from-profile profile.yml"})
+
[.no-copy]
----
...
```
// (step {"action":"runShell", "command": "rpk transform deploy --var=SCHEMA_ID=1 --input-topic=iss_json --output-topic=iss_avro"})
...
```
// (step {"action":"runShell", "command": "bash ../../setup-tests/tools/rpk-produce.sh", "args": ["iss_json", "{\"message\": \"success\", \"timestamp\": 1695753164, \"iss_position\": {\"latitude\": \"-12.8784\", \"longitude\": \"92.2935\"}}"]})
...
```
// (step {"action":"runShell", "command": "rpk topic consume iss_avro --num 1"})
+
Example output:
+
```json

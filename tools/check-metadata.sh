...
#     per-step specs are generated, never committed
...
for slug in $slugs; do
  ...
  if [ -z "$steps_raw" ]; then
    ...
    for s in "${steps[@]}"; do
      ...
      if [ "$s" = "step" ]; then
        err "$page" "step id 'step' is the template placeholder: rename pages/step.adoc and steps/step/ to the real step id"
      fi
      ...
      if [ -f "$code/tests/doc-detective/specs/$s.json" ]; then
        err "$code/tests/doc-detective/specs/$s.json" "per-step specs are generated by tools/gen-dd-specs.mjs; delete the committed file"
      fi
    done
    # The generated Doc Detective specs: every step page must yield at least one
    # runnable command block and one expected-output check, and every tag it
    # references must exist.
    if command -v node >/dev/null; then
      while IFS= read -r line; do
        case "$line" in
          "gen-dd-specs: ERROR "*) err "$slug" "${line#gen-dd-specs: ERROR }" ;;
        esac
      done < <(node "$root/tools/gen-dd-specs.mjs" "$slug" --check 2>&1 >/dev/null)
    else
      warn "$slug" "node not found; skipped tools/gen-dd-specs.mjs --check"
    fi
  ...
  # Page bodies: no literal code. Every listing block carries [source,<lang>] or
  # [,<lang>] and contains only include:: lines (blank lines allowed), so every
  # command and every expected output comes from the solution directory.
  while IFS= read -r f; do
    while IFS=$'\t' read -r ln msg; do
      ...
      function flush_attrs() { attrs = "" }
      {
        if (delim != "") {
          ...
          if ($0 ~ /^[[:space:]]*$/) next
          if ($0 !~ /^include::/) printf "%d\tliteral code in a listing block; show it with include::example$... from the solution directory instead\n", NR
          next
        }
        if ($0 == "----" || $0 == "....") {
          if (attrs !~ /(^|\n)\[(source,|,)[A-Za-z0-9_+-]+/) printf "%d\tlisting block without [source,<lang>] (or [,<lang>])\n", NR
          delim = $0; flush_attrs(); next
        }

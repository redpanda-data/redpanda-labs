...
NOTE: This approach avoids the known port-forwarding issues with MinIO console. The MinIO Console UI requires websockets which don't work reliably through `kubectl port-forward` tunnels. NodePort provides direct access without websocket connectivity issues.

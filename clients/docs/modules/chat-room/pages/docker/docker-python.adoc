...
- link:{content-url}/clients/chat-room/docker/python/admin.py[`admin.py`]: Checks whether the `chat-room` topic exists and creates it if not.
- link:{content-url}/clients/chat-room/docker/python/producer.py[`producer.py`]: A producer that sends strings entered by the user of the terminal to the
`chat-room` topic. Messages are sent as JSON encoded strings.
- link:{content-url}/clients/chat-room/docker/python/consumer.py[`consumer.py`]: A consumer that reads all messages from the `chat-room`
...
- link:{content-url}/clients/chat-room/docker/python/app.py[`app.py`]: The client application that creates the topic, producer, and consumer and implements the chat logic.
+

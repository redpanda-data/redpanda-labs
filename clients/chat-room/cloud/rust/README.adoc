...
:page-layout: lab
:page-aliases: 23.3@ROOT:develop:guide-rust-cloud.adoc
// Set the path to content in this lab for both the docs site and the local GitHub README
:content-url: ./
ifdef::env-site[]
:content-url: https://github.com/redpanda-data/redpanda-labs/blob/main/clients/chat-room/cloud/rust/
endif::[]
ifndef::env-site[]
:imagesdir: ../../../docs/modules/clients/images/
endif::[]

Create a basic chat room application with Redpanda Cloud and Kafka clients developed with https://github.com/twmb/fede1024/rust-rdkafka[rust-rdkafka^].
...
- Download and install Rust from https://rust-lang.org/tools/install[rust-lang.org^].

- Complete the link:https://docs.redpanda.com/cloud-data-platform/get-started/cluster-types/create-dedicated-cloud-cluster/[Redpanda Cloud Dedicated quickstart] before continuing. This example expands on the quickstart.
...
[,bash]
----
cd clients/chat-room/cloud/rust
----

. Rename the `.env.example` file to `.env` and replace the placeholders wrapped in angle brackets (`<>`) with the same values that you used in the Redpanda Cloud Quickstart.

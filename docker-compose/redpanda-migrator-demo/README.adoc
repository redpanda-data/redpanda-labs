...
----
+
This command confirms that the `migrator-user` SASL user has correct read-only source access and read-write target access with proper restrictions. See `scripts/verify-acls.sh` for test details.
...
+
This command starts a background producer that sends approximately 80 messages every 2 seconds across all demo topics.
...
----
+
This command displays a real-time dashboard showing lag per topic, message counts, and production status. Press kbd:[Ctrl+C] to exit.

. When the migration is finished, stop continuous production:
+
[,bash]
----
make demo-stop
...
Run the verification script to confirm a successful migration:

...
"""Show what the client knows about the cluster behind Envoy.
  ...
  endpoint.py [--expect-routing source|shadow] [--routing-only]
...
def main(argv):
    ...
    # A loop that is waiting for Envoy to change its mind asks for the current
    # answer, not a patient one.
    report, routing = common.envoy_routing(0 if routing_only else common.ENVOY_WAIT)
    if not routing_only:

...
```bash
npx doc-tools link-readme -s <path-to-readme> -t <target-asciidoc-filename>
```
...
- `page-categories`: Assigns <<categories, categories>> to your page. Use a comma-separated list for multiple categories. Categories are validated against a centralized list that the documentation team maintains. These categories are used to generate links to related docs and related labs as well as provide filters on the Redpanda Labs landing page.
...
The categories for Redpanda docs are centrally managed in a YAML file that the documentation team maintains.
...
The Redpanda docs team welcomes contributions and suggestions for improving or expanding the category definitions. If you have ideas for new categories or adjustments to existing ones that could enhance the organization and discoverability of content, https://github.com/redpanda-data/redpanda-labs/issues/new/choose[open an issue^] in this repository.

Describe the proposed category addition or modification, providing context on why the change is beneficial and how it fits within the overall documentation strategy. If you are unsure whether an existing category already covers what you need, say so in the issue. The docs team maintains the category list and makes the change.
